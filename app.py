...
def gerar_playbook_ia(d):
    ...
    - Setor de Atuação do Cliente: {d['segmento']}
    ...
    1. 🎯 ESTRATÉGIA MACRO (Uma frase forte conectando o direcional da conta ao negócio/setor do cliente)
    2. 🛠️ AÇÕES IMEDIATAS (3 tópicos curtos e acionáveis)
    3. 💡 INSIGHT DE EXPANSÃO (Como vender mais serviços como Segurança, Cloud ou Guardian, justificando com as dores típicas do setor de {d['segmento']})
...
with st.sidebar:
    ...
    # NOVO: SEGMENTO
    segmento = st.selectbox("Segmento do Cliente", ["Tecnologia / SaaS", "Saúde / Clínicas", "Varejo / E-commerce", "Indústria / Manufatura", "Serviços B2B / Consultoria", "Educação", "Financeiro / Contabilidade", "Outro"])
    ...
    if fase_jornada == 'Onboarding': st.info("🎯 **0-6 meses:** Foco em implementação e primeiro valor técnico.")
    elif fase_jornada == 'Adoção': st.info("⚙️ **6-24 meses:** Foco em uso recorrente e estabilidade técnica.")
    else: st.info("🤝 **+24 meses:** Parceria estratégica a longo prazo e novos negócios.")
...
with r1:
    with st.container(border=True):
        st.markdown("**Saúde do Serviço (Resumo)**")
        st.caption(f"**Volume:** {vol_chamados}<br>**SLA:** {sla_mes}%", unsafe_allow_html=True)
with r2:
    with st.container(border=True):
        ...
        if cohort in ["Diamante", "Ouro", "Prata"]: qbr_st = st.radio("QBR no Prazo?", ["Sim", "Não"], horizontal=True)
        else: qbr_st = "N/A"; st.caption("ℹ️ *QBR não aplicável para tier Bronze.*")
            
with r3:
    with st.container(border=True):
        st.markdown("**Satisfação Percebida**")
        tem_nps = st.toggle("Cliente respondeu NPS recente?", value=True)
        if tem_nps: nps_nota = st.slider("Nota NPS (0-10)", 0, 10, 8)
        else: nps_nota = None; st.warning("⚖️ Peso do NPS redistribuído.")
...
with p1:
    with st.container(border=True):
        ...
        st.caption("**80-100:** Top faturamento | **40-79:** Médio | **0-39:** Pequenas", unsafe_allow_html=True)
with p2:
    with st.container(border=True):
...
if st.button("PROCESSAR RECLASSIFICAÇÃO COM IA"):
    if not nome_cliente: st.error("⚠️ Por favor, insira o nome do cliente.")
        ...
        with res1:
            st.plotly_chart(create_gauge("Nível de Risco", risco_f, [{'range': [0, 40], 'color': "#22c55e"}, {'range': [40, 65], 'color': "#f97316"}, {'range': [65, 100], 'color': "#ef4444"}]), use_container_width=True)
        with res2:
            st.plotly_chart(create_gauge("Potencial Expansão", potencial_f, [{'range': [0, 40], 'color': "#94a3b8"}, {'range': [40, 75], 'color': "#189CD8"}, {'range': [75, 100], 'color': "#95C11F"}]), use_container_width=True)

        st.write("---")
        with st.spinner("🤖 Gemini AI gerando estratégia corporativa..."):
            ...
            st.markdown("### 📋 Diagnóstico Executivo de Negócios")
            st.markdown(f"""
            ...
            # SALVAR NO GOOGLE SHEETS COM COLUNAS ALINHADAS
            try:
                ...
                st.warning(f"⚠️ Erro ao salvar na planilha (Verifique as colunas). Detalhe: {e}")
